...
export const fetchCatalogue = createAsyncThunk(
    ...
    async (pageHandler: ICarPageHandler, thunkAPI) => {
        try {
            const response = await axios.post<ICatalogue>(CATALOGUE_API_URL, {page:pageHandler.page,name: pageHandler.name, sortState: pageHandler.sortState, isAskOrder:pageHandler.isAskOrder});
